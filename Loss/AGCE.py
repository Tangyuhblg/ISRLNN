'''Asymmetric Loss Functions for Noise-Tolerant
...
class AGCELoss(nn.Module):
    def __init__(self, num_classes=2, a=1, q=2, eps=1e-7, scale=1.):
...
if __name__ == '__main__':
    ...
    print('*' * 50)
    print('AGCE Accuracy:%.4f+%.4f' % (np.mean(Acc_list), np.std(Acc_list)))
    print('AGCE F-measure:%.4f+%.4f' % (np.mean(F_measure_list), np.std(F_measure_list)))
    print('AGCE G-mean:%.4f+%.4f' % (np.mean(G_mean_list), np.std(G_mean_list)))
    print('AGCE MCC:%.4f+%.4f' % (np.mean(MCC_list), np.std(MCC_list)))
    print('AGCE AUC:%.4f+%.4f' % (np.mean(AUC_list), np.std(AUC_list)))
    print('AGCE FPR:%.4f+%.4f' % (np.mean(FPR_list), np.std(FPR_list)))
    print('AGCE FNR:%.4f+%.4f' % (np.mean(FNR_list), np.std(FNR_list)))
