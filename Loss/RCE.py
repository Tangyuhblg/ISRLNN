...
eps = 1e-7
class RCELoss(nn.Module): # 简单的归一化操作来对噪声标签进行鲁棒性
...
if __name__ == '__main__':
    ...
    data_frame = np.array(pd.read_csv(r'D:\A论文实验\data\JIRA\wicket-1.3.0-beta2.csv'))
    ...
    print('*' * 50)
    print('RCE Accuracy:%.4f+%.4f' % (np.mean(ACC_list), np.std(ACC_list)))
    print('RCE F-measure:%.4f+%.4f' % (np.mean(F_measure_list), np.std(F_measure_list)))
    print('RCE G-mean:%.4f+%.4f' % (np.mean(G_mean_list), np.std(G_mean_list)))
    print('RCE MCC:%.4f+%.4f' % (np.mean(MCC_list), np.std(MCC_list)))
    print('RCE AUC:%.4f+%.4f' % (np.mean(AUC_list), np.std(AUC_list)))
    print('RCE FPR:%.4f+%.4f' % (np.mean(FPR_list), np.std(FPR_list)))
    print('RCE FNR:%.4f+%.4f' % (np.mean(FNR_list), np.std(FNR_list)))
