...
class NELoss(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        targets = 2 * targets - 1
        # 计算负指数损失
        loss = torch.min(torch.ones_like(outputs), torch.exp(-targets * outputs - self.mu))
        return loss.mean()


class NE_loss(nn.Module):
    def __init__(self, weights, n_classes, gamma, l1_lambda=0.01, reduction='mean'):
        super(NE_loss, self).__init__()
    ...
    def forward(self, input, target, model):
        total_samples = np.sum(self.weights)
        props = self.weights / total_samples

        # 计算损失的非线性项
        nonlinear_component = np.sqrt(
            (1 - np.linalg.norm(props, ord=2) ** 2) * self.n_classes / (self.n_classes - 1) + np.finfo(np.float64).eps)

        # 计算损失的极值项
        extremal_component = (1 - np.max(props)) * self.n_classes / (self.n_classes - 1)

        # 使用 gamma 平衡两部分损失
        ne_loss_value = min(self.gamma * nonlinear_component, extremal_component)

        # 生成与输入形状相同的标量张量，并启用梯度计算
        loss_tensor = torch.tensor(ne_loss_value, requires_grad=True, dtype=input.dtype, device=input.device)

        # 计算 L1 范数
        l1_norm = sum(p.abs().sum() for p in model.parameters())

        # 使用不就地操作将 L1 范数添加到损失函数中
        loss_tensor = loss_tensor + self.l1_lambda * l1_norm

        return loss_tensor


def calculate_weights(y, idx, n_classes):
    """
    计算当前节点的 weights，即各个类别的样本数量。

    参数:
    ----------
    y : array-like of shape (n_samples,)
        样本的标签数组，每个元素表示该样本的类别标签。
    ...
    y = y.astype(np.int64)
    # 计算当前节点中每个类别的样本数量
    weights = np.bincount(y[idx], minlength=n_classes)
...
if __name__ == '__main__':
    ...
    for kf, (train_index, test_index) in enumerate(kfold.split(data)):
        ...
        idx = np.array(range(y_train_clone.shape[0]))
        # print(idx)
        weights = calculate_weights(y_train_clone, idx, n_classes)
        # print(f"Weights: {weights}")

        net = ConvModule()
        criterion = NE_loss(weights, n_classes, 0.01)
    ...
    print('*' * 50)
    print('RLF Accuracy:%.4f+%.4f' % (np.mean(Acc_list), np.std(Acc_list)))
    print('RLF F-measure:%.4f+%.4f' % (np.mean(F_measure_list), np.std(F_measure_list)))
    print('RLF G-mean:%.4f+%.4f' % (np.mean(G_mean_list), np.std(G_mean_list)))
    print('RLF MCC:%.4f+%.4f' % (np.mean(MCC_list), np.std(MCC_list)))
    print('RLF AUC:%.4f+%.4f' % (np.mean(AUC_list), np.std(AUC_list)))
    print('RLF FPR:%.4f+%.4f' % (np.mean(FPR_list), np.std(FPR_list)))
    print('RLF FNR:%.4f+%.4f' % (np.mean(FNR_list), np.std(FNR_list)))
