...
class NELoss(nn.Module):
    ...
    def forward(self, outputs, targets):
        # 将targets转换为one-hot编码，即[64] -> [64, 2]
        targets_one_hot = F.one_hot(targets, num_classes=outputs.size(1)).float()

        # 计算负指数损失
        loss = torch.min(torch.ones_like(outputs), torch.exp(-targets_one_hot * outputs - self.mu))
        return loss.mean()
...
if __name__ == '__main__':
    ...
    data_frame = np.array(pd.read_csv(r'D:\A论文实验\data\AEEEM\JDT.csv'))
