...
class ActiveNegativeLoss(torch.nn.Module):
    def __init__(self, active_loss, negative_loss,
                 alpha=1., beta=1., delta=5e-5) -> None:
        super().__init__()
...
if __name__ == '__main__':
    ...
    data_frame = np.array(pd.read_csv(r'../data/prop-1-92.csv'))
    ...
    print('*' * 50)
    print('param=1 ISRL G-mean:%.4f+%.4f'%(np.mean(G_mean_list), np.std(G_mean_list)))
    print('param=1 ISRL AUC:%.4f+%.4f'%(np.mean(AUC_list), np.std(AUC_list)))
    print('param=1 ISRL FPR:%.4f+%.4f'%(np.mean(FPR_list), np.std(FPR_list)))
    print('param=1 ISRL FNR:%.4f+%.4f'%(np.mean(FNR_list), np.std(FNR_list)))
